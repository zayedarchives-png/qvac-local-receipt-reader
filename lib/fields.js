...
function findDate(rows) {
  for (const row of rows) {
    ...
    if (date) return { ...date, confidence: row.confidence };
  }
  return null;
}

// Low-confidence text is kept, not thrown away: dropping one shaky block
// (say the amount next to "TOTAL") would lose the whole field. Instead each
// field is flagged `uncertain` when it rests on text below `minConfidence`.
export function extractFields(rows, { minConfidence = 0 } = {}) {
  const flag = (field) => field && { ...field, uncertain: field.confidence < minConfidence };
  return {
    merchant: flag(findMerchant(rows)),
    date: flag(findDate(rows)),
    total: flag(findTotal(rows)),
  };
}
